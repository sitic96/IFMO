...
/**----------------------------------------------------------------------------
                            readdip
-------------------------------------------------------------------------------
Считывания значения с DIP_SW8

Вход:       нет
Выход:      8-битное значение c переключателей DIP_SW8
Результат:  нет
Описание:

Пример:     Переведём порт ввода-вывода в Z-состояние для чтения
            данных DIP-переключателя, а затем считаем их.
-----------------------------------------------------------------------------*/
unsigned char readdip()
{
	unsigned char dipData;

	write_max( ENA,0x00 ); //Входим в режим для чтения
	dipData = read_max( EXT_LO ); //Считываем значения DIP-переключателя
